  ...
  if (newDataReady) {
    if (millis() > t + serialPrintInterval) {
      ...
      if(i<80 & eigthseconds()%80 == 0){ //Condition to set new tare every 1000 milliseconds
        ...
        // tell the ESP32 Cam that the data collection session has ended
        if(send){
          SerialPort.println("done"); // send to CAM
          send = false;
        }
      }
          ...
          if (meas>250 & meas < 350){
            float k = Eest/(Eest+Emeas);
            Est = Est + k*(meas-Est);
            Eest = (1-k)*(Eest);
            Serial.println("Estimated Val: ");
            Serial.println(Est);
          }

          digitalWrite(2, HIGH); // show data sending using onboard blue LED
          SerialPort.println(String(i)); // send to CAM
