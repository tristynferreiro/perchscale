...
void loop() {
  ...
  if (newDataReady) {
      ...
      if(reading<10 & eigthseconds%80 == 0){ //Condition to set new tare every 1000 milliseconds
      ...
      else if (reading>10){ // If a bird or heavy object is detected on the scale
  ...
  if (Serial.available() > 0) {
    ...
    else if (inByte == 'r') readFile(SD, file_name_path); // read the file
    else if (inByte == 'd') deleteFile(SD, file_name_path); // delete the file
    else if (inByte == 'w') writeFile(SD, file_name_path, "Start\n"); // write the file
  }
